...
pygame.display.set_caption('Easiest Game')
                ...
                for row in data:
                        ...
                        for tile in row:
                                if tile == 1:
                                        ...
                                        dirt = Dirt(col_count * tile_size, row_count * tile_size)
                                        dirt_group.add(dirt)
                                if tile == 2:
                                        ...
                                        tile = (img, img_rect)
                                        self.tile_list.append(tile)
                                        grass = Grass(col_count * tile_size, row_count * tile_size)
...
class Enemy(pygame.sprite.Sprite):
        ...
        def update(self):
                ...
                if pygame.sprite.spritecollide(self, dirt_group, False):
                        self.move_direction *= -1
                if not pygame.sprite.spritecollide(self, grass_group, False):
                        self.move_direction *= -1
...
class Grass1(pygame.sprite.Sprite):
        def __init__(self, x, y):
                pygame.sprite.Sprite.__init__(self)
                img = pygame.image.load('grass1.jpg')
                self.image = pygame.transform.scale(img, (tile_size, tile_size))
                self.rect = self.image.get_rect()
                self.rect.x = x
                self.rect.y = y
...
player = Player(40, screen_height - 80)
...
while run:
        ...
        if main_menu == True:
                ...
                draw_text('COLLECT ALL COINS TO WIN!', font1, green, (tile_size + 60), (tile_size - 25))
